...
export async function POST(request: NextRequest, routeContext: { params: Promise<{ taskId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(await context.store.claimTask({ actorMemberProfileId: context.memberProfileId, taskId, idempotencyKey: String(body.idempotencyKey ?? crypto.randomUUID()) }));
  } catch (error) {
