...
export async function POST(request: NextRequest, routeContext: { params: Promise<{ taskId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(await context.store.completeTask({ actorMemberProfileId: context.memberProfileId, taskId, completionNotes: typeof body.completionNotes === "string" ? body.completionNotes : undefined, idempotencyKey: String(body.idempotencyKey ?? crypto.randomUUID()) }));
  } catch (error) {
