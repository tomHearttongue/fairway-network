...
export async function POST(request: NextRequest, context: { params: Promise<{ reservationId: string }> }) {
  ...
  try {
    ...
    if (memberContext.kind === "setup_required") {
      return NextResponse.json({ error: "ENVIRONMENT_NOT_CONFIGURED", required: requiredEnvironmentVariables(), missing: memberContext.missing }, { status: 503 });
    }
    if (memberContext.kind === "unauthenticated") {
      return NextResponse.json({ error: "UNAUTHENTICATED" }, { status: 401 });
    }

    const result = await memberContext.store.cancelReservation({
      memberProfileId: memberContext.memberProfileId,
      reservationId,
      idempotencyKey: String(body.idempotencyKey ?? `cancel:${reservationId}`),
    });
