...
const root = path.join(repoRoot, "artifacts", "du1-remediation-r4-review");
const staging = path.join(root, "bundle-staging");
const zipPath = path.join(root, "fairway-du1-remediation-r4-review.zip");
const verifyRoot = path.join(root, "bundle-verify");
const rejectedZip = path.join(repoRoot, "artifacts", "du1-remediation-r3-review", "fairway-du1-remediation-r3-review.zip");
const rejectedZipSha256 = "539b3062374cfc2a134a8516125cad8aed51e6994fd09813aecfa8bcc81242cf";
const rejectedZipBytes = 2_176_897;
...
if (branch !== "du1/acceptance-remediation-r4") throw new Error(`Unexpected review branch: ${branch}`);
...
writeJson(path.join(staging, "provenance.json"), {
  ...
  slice: "DU1 Acceptance Remediation Round 4",
  ...
  rejectedCandidate: {
    commitSha: "37fa7d8f7e81718baef515237683436496a0a592",
...
const manifest = {
  schemaVersion: "fairway-du1-r4-review-v1",
  project: "Fairway Network",
  review: "DU1 Acceptance Remediation Round 4",
  status: "PENDING HUMAN REVIEW",
  commitSha,
  branch,
  generatedAt,
...
assertRejectedCandidateFrozen();
assertCleanTrackedTree();
if (gitText(["rev-parse", "HEAD"]) !== commitSha) throw new Error("HEAD changed while generating the DU1 R4 review package.");
console.log(JSON.stringify(result, null, 2));

function validateEvidence(input) {
  if (input.screenshots.length !== 31) throw new Error(`Expected 31 fresh screenshots, found ${input.screenshots.length}.`);
  ...
  let stateBackedUiAssertionCount = 0;
  let derivedPresentationAssertionCount = 0;
  let interactionAssertionCount = 0;
  let backendStateAssertionCount = 0;
  ...
  for (const item of input.screenshots) {
    ...
    if (item.structuredAssertions.some((assertion) => assertion.passed !== true || !assertion.id || !assertion.sourcePath || !assertion.comparator || !assertion.evidenceType || !assertion.sourceKind)) {
    ...
    for (const assertion of item.structuredAssertions) {
      ...
      if (assertion.sourceKind === "state-backed") {
        if (!resolveSourcePath(reconciliation.snapshot, assertion.sourcePath).found) {
          throw new Error(`State-backed assertion source path is missing: ${item.file} ${assertion.id} -> ${assertion.sourcePath}`);
        }
        if (assertion.evidenceType === "ui") stateBackedUiAssertionCount += 1;
        else backendStateAssertionCount += 1;
      } else if (assertion.sourceKind === "derived-presentation") {
        derivedPresentationAssertionCount += 1;
      } else if (assertion.sourceKind === "interaction") {
        interactionAssertionCount += 1;
      }
    }
  ...
  for (const item of mobileSteps) {
    ...
    const present = new Set(item.structuredAssertions.map((assertion) => assertion.sourcePath));
    for (const id of required) {
      if (!present.has(id.replace(/^(?:ui|state):/, ""))) missingRequiredAssertionCount += 1;
    }
  }
  ...
  if (uniqueLogicalStates !== 25) throw new Error(`Expected 25 unique logical states, found ${uniqueLogicalStates}.`);
  const postCompletion = input.screenshots.find((item) => item.state === "runtime-completion-projected-once");
  if (!postCompletion) throw new Error("Supplemental post-completion My Golf evidence is missing.");
  for (const sourcePath of [
    "golfProfile.completedSessionCount",
    "golfProfile.recentActivityCount",
    "golfProfile.latestActivityDetail",
    "golfProfile.driverCarryLabel",
    "golfProfile.sevenIronCarryLabel",
    "golfProfile.pitchingWedgeCarryLabel",
  ]) {
    if (!postCompletion.structuredAssertions.some((assertion) => assertion.sourcePath === sourcePath)) {
      throw new Error(`Post-completion My Golf evidence lacks ${sourcePath}.`);
    }
  }
  return {
    screenshotCaptures: input.screenshots.length,
    uniqueLogicalStates,
    viewportConfigurations: viewportCounts,
    structuredAssertionCount,
    stateBackedUiAssertionCount,
    derivedPresentationAssertionCount,
    interactionAssertionCount,
    backendStateAssertionCount,
...
function requiredAssertionIdsFor(stepId) {
  ...
  "golden-05-my-golf": [
    ...
    "ui:golfProfile.completedSessionCount",
    "ui:golfProfile.recentActivityCount",
    "ui:golfProfile.latestActivityTitle",
    "ui:golfProfile.liveProviderImplicationAbsent",
    "state:golfProfile.completedSessionCount",
    "state:golfProfile.recentActivityCount",
  ],
  "golden-06-session-completion": [
    "ui:member.firstName",
    "ui:completedLifecycle.suiteName",
    "ui:completedLifecycle.bookingLabel",
    ...
    "state:completedLifecycle.scheduledDurationMinutes",
    "state:completedLifecycle.elapsedDurationMinutes",
    "state:completedLifecycle.sessionStartedAt",
    "state:completedLifecycle.sessionEndedAt",
  ...
  "golden-07-completed-lifecycle": [
    ...
    "ui:futureReservation.startLabel",
    "ui:futureReservation.accessOpensLabel",
    "ui:completedLifecycle.completedSuiteAbsentFromUpNext",
    "state:completedLifecycle.reservationStatus",
    "state:completedLifecycle.accessStatus",
    "state:completedLifecycle.turnoverTaskStatus",
  ],
  "golden-08-facilities-queue": [
    ...
    "state:topTask.sourceSessionPresent",
...
function writeSourceSnapshot(commit) {
  const include = [
    ...
    /^supabase\/migrations\//,
    /^scripts\/(?:demo-|run-du1-|verify-du1-|du1-(?:r4-)?review-bundle)/,
    /^tests\/domains\/(?:demo-universe|du1-round2|du1-round3|du1-round4)/,
...
function verifyStaging(directory, expectedCommit) {
  const required = [
    ...
    "reports/member-activity-completion.json",
    ...
    "source/src/application/member-flow/member-activity.ts",
    "source/src/application/member-flow/demo-golf-profile.ts",
    "source/app/api/member/availability/route.ts",
...
function assertCleanTrackedTree() {
  const dirty = gitText(["status", "--porcelain", "--untracked-files=no"]).split(/\r?\n/).filter(Boolean);
   if (dirty.length) throw new Error(`Cannot generate DU1 R4 Product Acceptance package: tracked tree is dirty.\n${dirty.join("\n")}`);
}
...
function resolveSourcePath(value, sourcePath) {
  let current = value;
...
function readme({ commitSha, branch, generatedAt, audit, evidenceSummary, sourceSnapshot }) {
   return `# Fairway Network DU1 Acceptance Remediation Round 4
