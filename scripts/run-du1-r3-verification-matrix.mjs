...
try {
  ...
  assertCleanTrackedTree();
  if (gitText(["rev-parse", "HEAD"]) !== commitSha) throw new Error("HEAD changed during DU1 R3 verification.");
  writeMatrix();
  console.log(`DU1 R3 verification matrix: PASS (${gates.length} gates plus Experience QA)`);
} catch (error) {
...
function assertCleanTrackedTree() {
  ...
  if (dirty.length) throw new Error(`DU1 R3 verification requires a clean tracked tree.\n${dirty.join("\n")}`);
}
