...
const commands = [
  ...
  gate("focused-round-3-tests", "pnpm exec vitest run tests/domains/du1-round3-remediation.test.ts", process.execPath, ["node_modules/vitest/vitest.mjs", "run", "tests/domains/du1-round3-remediation.test.ts"]),
  gate("focused-round-4-tests", "pnpm exec vitest run tests/domains/du1-round4-remediation.test.ts", process.execPath, ["node_modules/vitest/vitest.mjs", "run", "tests/domains/du1-round4-remediation.test.ts"]),
  ...
  gate("temporal-authority", "pnpm verify:du1:r3:temporal-authority", process.execPath, ["scripts/verify-du1-r3-temporal-authority.mjs"]),
  gate("member-activity-completion", "pnpm verify:du1:r4:member-activity", process.execPath, ["scripts/verify-du1-r4-member-activity.mjs"]),
];
...
try {
  ...
  assertCleanTrackedTree();
  if (gitText(["rev-parse", "HEAD"]) !== commitSha) throw new Error("HEAD changed during DU1 R4 verification.");
  writeMatrix();
  console.log(`DU1 R4 verification matrix: PASS (${gates.length} gates plus Experience QA)`);
} catch (error) {
...
function assertCleanTrackedTree() {
  ...
  if (dirty.length) throw new Error(`DU1 R4 verification requires a clean tracked tree.\n${dirty.join("\n")}`);
}
