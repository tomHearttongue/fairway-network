...
import { selectHomeFeaturedReservation, sessionCompletionPresentation } from "@/application/member-flow/member-activity";
import { formatLocationDate, formatLocationDateTime, formatLocationTime } from "@/shared/location-time";
...
type CompletionSummary = { reservationId: string; memberFirstName: string; suiteName: string; scheduledDurationMinutes: number; elapsedDurationMinutes: number; sessionStartedAt: string; sessionEndedAt: string };
...
function AuthenticatedFlow() {
  ...
  const selectedReservation = useMemo(() => {
    const explicit = data?.reservations.find((reservation) => reservation.id === selectedReservationId && reservation.status !== "completed" && reservation.status !== "cancelled");
    return explicit ?? activeSession ?? immediateReservation ?? upcomingReservations[0];
  }, [activeSession, data, immediateReservation, selectedReservationId, upcomingReservations]);
  const homeFeaturedReservation = useMemo(
    () => selectHomeFeaturedReservation(data?.reservations ?? [], selectedReservationId),
  ...
  async function refresh(): Promise<AvailabilityResponse> { const response = await fetch("/api/member/availability", { cache: "no-store" }); const body = await response.json(); if (!response.ok) throw new Error(friendlyError(body.error, "We could not refresh Fairway. Try again in a moment.")); setData(body); return body; }
  ...
  async function createPlayNowReservation() {
    ...
    setSelectedReservationId(body.reservation.id); changeView("play");
    const refreshed = await refresh();
    const createdReservation = refreshed.reservations.find((reservation) => reservation.id === body.reservation.id);
    setLoading(false);
    setResult({
      ok: true,
      reservationId: body.reservation.id,
      message: `${createdReservation?.suiteName ?? quoteContext?.suiteName ?? "Your Practice Suite"} is ready. ${createdReservation ? memberAccessMessage(createdReservation, refreshed.location.timezone) : "Access will be prepared for this session."}`,
    });
  }
  ...
  async function completeSession() {
    ...
    const completion = sessionCompletionPresentation({
      scheduledStartAt: reservation?.startAt ?? body.reservation.startAt,
      scheduledEndAt: reservation?.endAt ?? body.reservation.endAt,
      sessionStartedAt: body.session.startedAt,
      sessionEndedAt: body.session.endedAt,
    });
    setSelectedReservationId(null);
    setCompletionSummary({ reservationId, memberFirstName: data ? memberFirstName(data.member.person.displayName, data.demoGolfProfile.displayName) : "there", suiteName: reservation?.suiteName ?? "Your Practice Suite", ...completion });
  ...
  return (
    <div className="fairway-member">
      ...
      <main className="member-main">
        ...
        {view === "home" && <HomeView data={data} canPlayNow={canPlayNow} featuredReservation={homeFeaturedReservation} activeSession={activeSession} loading={loading} quote={data.playNowQuote} selectedOption={selectedPlayNowOption} immediateReservation={immediateReservation} onDurationChange={setSelectedDuration} onPlayNow={createPlayNowReservation} onStartSession={startSession} onCompleteSession={completeSession} onOpenPlay={() => changeView("play")} onOpenGolf={() => changeView("golf")} />}
        {view === "play" && <PlayView data={data} selectedReservation={selectedReservation} upcomingReservations={upcomingReservations} historicalReservations={historicalReservations} quote={data.playNowQuote} selectedOption={selectedPlayNowOption} canPlayNow={canPlayNow} loading={loading} guestName={guestName} guestEmail={guestEmail} guestLimitReached={guestLimitReached} immediateReservation={immediateReservation} completionSummary={completionSummary} onDurationChange={setSelectedDuration} onPlayNow={createPlayNowReservation} onStartSession={startSession} onCompleteSession={completeSession} onDismissCompletionSummary={() => { setSelectedReservationId(null); setCompletionSummary(null); changeView("home"); }} onViewGolfFromSummary={() => { setSelectedReservationId(null); setCompletionSummary(null); changeView("golf"); }} onCancel={cancelReservation} onSelectReservation={setSelectedReservationId} onGuestNameChange={setGuestName} onGuestEmailChange={setGuestEmail} onAddGuest={addGuest} onGuestAction={mutateGuest} />}
        {view === "golf" && <MyGolfView profile={data.demoGolfProfile} timeZone={data.location.timezone} />}
      </main>
    </div>
  );
}
...
function MemberContext({ headingRef, view, data, canPlayNow, selectedReservation }: { headingRef: RefObject<HTMLHeadingElement | null>; view: MemberView; data: AvailabilityResponse; canPlayNow: boolean; selectedReservation?: MemberReservation }) {
  ...
  const copy = view === "home"
    ? canPlayNow ? "Choose a duration and Fairway will assign the right Practice Suite." : selectedReservation ? `Next up: ${selectedReservation.suiteName} at ${formatLocationDateTime(selectedReservation.startAt, data.location.timezone)}.` : "No suite is ready right now."
...
function HomeView({ data, canPlayNow, featuredReservation, activeSession, immediateReservation, loading, quote, selectedOption, onDurationChange, onPlayNow, onStartSession, onCompleteSession, onOpenPlay, onOpenGolf }: { data: AvailabilityResponse; canPlayNow: boolean; featuredReservation?: MemberReservation; activeSession?: MemberReservation; immediateReservation?: MemberReservation; loading: boolean; quote?: PlayNowQuote; selectedOption?: PlayNowQuoteOption; onDurationChange: (value: number) => void; onPlayNow: () => void; onStartSession: () => void; onCompleteSession: () => void; onOpenPlay: () => void; onOpenGolf: () => void }) {
  const attentionGuest = featuredReservation?.guests.find((guest) => guest.status === "active" && !guest.ready);
  return <section className="home-layout" aria-label="Fairway home"><PlayNowPanel quote={quote} selectedOption={selectedOption} immediateReservation={immediateReservation} timeZone={data.location.timezone} loading={loading} canPlayNow={canPlayNow} onDurationChange={onDurationChange} onPlayNow={onPlayNow} onStartSession={onStartSession} onCompleteSession={onCompleteSession} /><article className="session-card"><div className="section-heading"><span><DoorOpen size={18} />{activeSession ? "Current session" : "Up next"}</span></div>{featuredReservation ? <SessionState reservation={featuredReservation} timeZone={data.location.timezone} loading={loading} showActions={!immediateReservation} onStartSession={onStartSession} onCompleteSession={onCompleteSession} /> : <EmptyState title="No reservation yet" body="Play Now is the fastest path when a suite is ready." action="Open Play" onAction={onOpenPlay} />}</article><article className="insight-card"><div className="section-heading"><span><Target size={18} />My Golf</span><button className="text-button" type="button" onClick={onOpenGolf}>View</button></div><PassportPreview profile={data.demoGolfProfile} /></article>{attentionGuest && <article className="attention-card"><div className="section-heading"><span><Flag size={18} />Guest readiness</span></div><p><strong>{attentionGuest.displayName}</strong> still needs waiver completion before they are ready for this reservation.</p></article>}</section>;
}
...
function SessionState({ reservation, timeZone, loading, showActions = true, onStartSession, onCompleteSession }: { reservation: MemberReservation; timeZone: string; loading: boolean; showActions?: boolean; onStartSession: () => void; onCompleteSession: () => void }) {
  ...
  const canStart = Boolean(reservation.canStartSession);
  const canFinish = reservation.status === "checked_in" && Boolean(reservation.canCompleteSession);
  return <div className="session-state-card"><div><span className="state-label">{reservationStatusLabel(reservation)}</span><h3>{reservation.suiteName}</h3><p>{formatLocationDateTime(reservation.startAt, timeZone)} to {formatLocationTime(reservation.endAt, timeZone)}</p></div><dl className="state-list"><div><dt>Access</dt><dd>{accessStatusCopy(reservation, timeZone)}</dd></div><div><dt>Booking</dt><dd>{bookingModeLabel(reservation.bookingMode)}</dd></div><div><dt>Guests</dt><dd>{activeGuests === 0 ? "None" : `${readyGuests}/${activeGuests} ready`}</dd></div></dl>{showActions && canStart && <div className="action-stack horizontal"><button className="primary-action" type="button" disabled={loading} onClick={onStartSession}><DoorOpen size={18} />Start session</button></div>}{showActions && canFinish && <div className="action-stack horizontal"><button className="primary-action" type="button" disabled={loading} onClick={onCompleteSession}><CheckCircle2 size={18} />Finish session</button></div>}{showActions && reservation.status === "confirmed" && !canStart && <SessionStartAvailability reservation={reservation} timeZone={timeZone} />}{!canStart && !canFinish && reservation.status === "completed" && <p className="supporting-copy">Session complete. Your activity is saved.</p>}</div>;
}
function SessionStartAvailability({ reservation, timeZone }: { reservation: MemberReservation; timeZone: string }) {
  if (reservation.startBlockedReason === "access_scheduled" && reservation.accessGrant?.startsAt) {
    return <p className="supporting-copy">Start Session becomes available when access opens at {formatLocationTime(reservation.accessGrant.startsAt, timeZone)}.</p>;
  }
  if (reservation.startBlockedReason === "access_expired" || reservation.startBlockedReason === "access_revoked") {
    return <p className="supporting-copy">This session can no longer be started from the member app.</p>;
  }
  return null;
}
function ReservationList(props: { title: string; reservations: MemberReservation[]; timeZone: string; selectedReservationId?: string; loading: boolean; onSelect: (reservationId: string) => void; onCancel: (reservationId: string) => void }) {
...
function reservationStatusLabel(reservation: MemberReservation): string { if (reservation.status === "checked_in") return "Session active"; if (reservation.status === "confirmed" && reservation.accessWindowStatus === "scheduled") return "Booked"; if (reservation.status === "confirmed" && reservation.canStartSession) return "Ready to start"; if (reservation.status === "confirmed") return "Booked"; if (reservation.status === "completed") return "Completed"; if (reservation.status === "cancelled") return "Cancelled"; return "Held"; }
...
function accessStatusCopy(reservation: MemberReservation, timeZone: string): string { if (reservation.accessWindowStatus === "scheduled" && reservation.accessGrant?.startsAt) return `Opens at ${formatLocationTime(reservation.accessGrant.startsAt, timeZone)}`; return accessStatusLabel(reservation.accessWindowStatus); }
