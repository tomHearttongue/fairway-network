...
import { CalendarPlus, CheckCircle2, Clock3, DoorOpen, FileCheck2, Flag, Gauge, History, MapPin, Play, RefreshCcw, ShieldCheck, Target, Trash2, UserPlus, Users, XCircle } from "lucide-react";
...
function AuthenticatedFlow() {
  ...
  async function createReservation(mode: "ADVANCE" | "PLAY_NOW") {
    ...
    setSelectedReservationId(body.reservation.id); setView("play"); await refresh(); setLoading(false); setResult({ ok: true, reservationId: body.reservation.id, message: `${mode === "PLAY_NOW" ? "You're ready to play." : "Your next safe slot is booked."} ${suiteDisplay(body.reservation.suiteId)} is assigned. Access opens ${formatTime(body.accessGrant.startsAt)}.` });
  }
  ...
  async function addGuest() {
    ...
    setGuestName(""); setGuestEmail(""); await refresh(); setLoading(false); setResult({ ok: true, reservationId: selectedReservation.id, message: `${body.reservationGuest.displayName} is added. Their waiver needs to be completed before they are ready.` });
  }
  ...
  async function mutateGuest(reservationGuestId: string, action: "request" | "complete" | "eligibility" | "remove") {
    ...
    const messages = { request: "Waiver request recorded.", complete: body.ready ? "Waiver complete. Guest is ready when the access window is active." : "Waiver recorded, but the guest is not ready yet.", eligibility: body.accessEligible ? "Guest is ready for the active access window." : `Guest is not ready yet: ${friendlyGuestBlock(body.blockedReason)}.`, remove: "Guest removed from this reservation." };
  ...
  return (
    <div className="fairway-member">
      ...
      <main className="member-main">
        ...
        <MemberContext view={view} data={data} canPlayNow={canPlayNow} selectedReservation={selectedReservation} />
...
function MemberContext({ view, data, canPlayNow, selectedReservation }: { view: MemberView; data: AvailabilityResponse; canPlayNow: boolean; selectedReservation?: MemberReservation }) {
  const firstName = memberFirstName(data.member.person.displayName);
  const title = view === "home" ? `Ready to play, ${firstName}?` : view === "play" ? "Play" : "My Golf";
  const copy = view === "home"
    ? canPlayNow ? "A Practice Suite is ready now." : selectedReservation ? `Next up: ${selectedReservation.suiteName} at ${formatDateTime(selectedReservation.startAt)}.` : "No suite is ready right now. You can still reserve the next safe slot."
    : view === "play" ? "Start now, manage the next reservation, or bring a guest into this session."
    : "Your Fairway golf identity, baselines, and activity live here.";
  return <section className={`member-context ${view !== "home" ? "compact" : ""}`} aria-labelledby="member-home-title"><div><p className="eyebrow">{data.location.name}</p><h1 id="member-home-title">{title}</h1><p>{copy}</p></div><div className="member-summary" aria-label="Member summary"><span className="demo-badge">Demo</span><span><strong>{data.member.availableCredits}</strong> credits</span><span>{planLabel(data.member.membershipPlan.code)}</span></div></section>;
}
...
function GuestTile({ guest, loading, onGuestAction }: { guest: MemberReservationGuest; loading: boolean; onGuestAction: (reservationGuestId: string, action: "request" | "complete" | "eligibility" | "remove") => void }) {
  const canRequest = guest.status !== "removed" && guest.waiverStatus === "not_requested";
  const canComplete = guest.status !== "removed" && guest.verificationState !== "verified";
  return <div className={`guest-tile ${guest.ready ? "ready" : "pending"}`}><div><strong>{guest.displayName}</strong><span>{guestStatusLabel(guest)}</span></div><div className="guest-actions">{canRequest && <button className="quiet-button" type="button" disabled={loading} onClick={() => onGuestAction(guest.id, "request")}><FileCheck2 size={16} />Request waiver</button>}{canComplete && <button className="quiet-button" type="button" disabled={loading} onClick={() => onGuestAction(guest.id, "complete")}><ShieldCheck size={16} />Mark complete</button>}<button className="quiet-button" type="button" disabled={loading || guest.status === "removed"} onClick={() => onGuestAction(guest.id, "eligibility")}>Check readiness</button><button className="danger-action" type="button" disabled={loading || guest.status === "removed"} onClick={() => onGuestAction(guest.id, "remove")}><Trash2 size={16} />Remove</button></div></div>;
}
