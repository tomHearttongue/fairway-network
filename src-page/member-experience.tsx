...
function PlayNowPanel({ quote, selectedOption, immediateReservation, loading, canPlayNow, onDurationChange, onPlayNow, onStartSession, onCompleteSession }: { quote?: PlayNowQuote; selectedOption?: PlayNowQuoteOption; immediateReservation?: MemberReservation; loading: boolean; canPlayNow: boolean; onDurationChange: (value: number) => void; onPlayNow: () => void; onStartSession: () => void; onCompleteSession: () => void }) {
  if (immediateReservation) {
    return <article className="play-now-card ready-card"><div className="availability-pill"><span className="dot ready" />{immediateReservation.status === "checked_in" ? "Session active" : "You are ready"}</div><h2>{immediateReservation.suiteName}</h2><p>{accessReadyCopy(immediateReservation)} Session runs until {formatTime(immediateReservation.endAt)}.</p><div className="action-stack">{immediateReservation.canStartSession && <button className="primary-action large" type="button" disabled={loading} onClick={onStartSession}><DoorOpen size={20} />Start Session</button>}{immediateReservation.status === "checked_in" && immediateReservation.canCompleteSession && <button className="primary-action large" type="button" disabled={loading} onClick={onCompleteSession}><CheckCircle2 size={20} />Finish Session</button>}</div>{immediateReservation.status === "confirmed" && !immediateReservation.canStartSession && <SessionStartAvailability reservation={immediateReservation} />}</article>;
  }
...
function SessionState({ reservation, loading, showActions = true, onStartSession, onCompleteSession }: { reservation: MemberReservation; loading: boolean; showActions?: boolean; onStartSession: () => void; onCompleteSession: () => void }) {
  ...
  return <div className="session-state-card"><div><span className="state-label">{reservationStatusLabel(reservation)}</span><h3>{reservation.suiteName}</h3><p>{formatDateTime(reservation.startAt)} to {formatTime(reservation.endAt)}</p></div><dl className="state-list"><div><dt>Access</dt><dd>{accessStatusCopy(reservation)}</dd></div><div><dt>Booking</dt><dd>{bookingModeLabel(reservation.bookingMode)}</dd></div><div><dt>Guests</dt><dd>{activeGuests === 0 ? "None" : `${readyGuests}/${activeGuests} ready`}</dd></div></dl>{showActions && canStart && <div className="action-stack horizontal"><button className="primary-action" type="button" disabled={loading} onClick={onStartSession}><DoorOpen size={18} />Start session</button></div>}{showActions && canFinish && <div className="action-stack horizontal"><button className="primary-action" type="button" disabled={loading} onClick={onCompleteSession}><CheckCircle2 size={18} />Finish session</button></div>}{showActions && reservation.status === "confirmed" && !canStart && <SessionStartAvailability reservation={reservation} />}{!canStart && !canFinish && reservation.status === "completed" && <p className="supporting-copy">Session complete. Your activity is saved.</p>}</div>;
}
function SessionStartAvailability({ reservation }: { reservation: MemberReservation }) {
  if (reservation.startBlockedReason === "access_scheduled" && reservation.accessGrant?.startsAt) {
    return <p className="supporting-copy">Start Session becomes available when access opens at {formatTime(reservation.accessGrant.startsAt)}.</p>;
  }
...
function accessStatusCopy(reservation: MemberReservation): string { if (reservation.accessWindowStatus === "scheduled" && reservation.accessGrant?.startsAt) return `Opens at ${formatTime(reservation.accessGrant.startsAt)}`; return accessStatusLabel(reservation.accessWindowStatus); }
